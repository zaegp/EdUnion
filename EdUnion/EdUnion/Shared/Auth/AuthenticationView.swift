...
struct AuthenticationView: View {
    ...
    private func handleSignInWithApple(result: Result<ASAuthorization, Error>) {
        ...
        case .success(let authResults):
            handleAuthorizationSuccess(authResults: authResults)
        case .failure(let error):
            print("failure", error)
        }
    }
    
    private func handleAuthorizationSuccess(authResults: ASAuthorization) {
        switch authResults.credential {
        case let appleIDCredential as ASAuthorizationAppleIDCredential:
            processAppleIDCredential(appleIDCredential)
        case let passwordCredential as ASPasswordCredential:
            processPasswordCredential(passwordCredential)
        default:
            break
        }
    }
    ...
    private func processPasswordCredential(_ passwordCredential: ASPasswordCredential) {
        let username = passwordCredential.user
        let password = passwordCredential.password
        print(username, password)
    }
    
    private func signInWithCredential(_ credential: OAuthCredential, appleIDCredential: ASAuthorizationAppleIDCredential) {
        Auth.auth().signIn(with: credential) { (authResult, error) in
            ...
            checkUserExistence(uid: uid, appleIDCredential: appleIDCredential)
        }
    }
